	...
	var upload = {
		...
		init: function() {
			...
			if (that.input) {
				...
				var changeUpload=function(e) {
					that.getFiles(e);
					if (that.auto) that.go();
				}
				that.input.removeEventListener("change",changeUpload, false);
				that.input.addEventListener("change",changeUpload, false);
			}
			//上传按钮提交
			if (that.button) {
				var buttonUpload=function(e) {
					that.go();
				};
				that.button.removeEventListener("click",buttonUpload, false);
				that.button.addEventListener("click",buttonUpload, false);
			}
		}
